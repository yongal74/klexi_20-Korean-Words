# Twenty Korean - Marketing Content Kit

---

## Reddit Posts

### r/Korean Post
**Title:** I built a free Korean learning app with AI conversation practice — 20 words/day, TOPIK curriculum

**Body:**
Hey everyone! I've been working on a Korean learning app called Twenty Korean and wanted to share it with the community.

The concept is simple: learn just 20 high-frequency Korean words per day, organized by TOPIK levels. In 360 days, you cover 7,200 essential words across all 6 TOPIK levels.

What makes it different:
- **AI Korean Tutor "Dalli"** — practice real conversations in Korean, and Dalli responds with natural voice
- **K-Culture Themes** — learn vocabulary through K-Drama, K-Pop, K-Food, Travel, Internet Slang
- **Word Network** — see how Korean words connect through meaning (synonyms, antonyms, compounds)
- **Complete Hangeul** — learn all 40 characters with pronunciation guides and syllable formation

It's free to start (Level 1 + Hangeul + 5 AI chat messages). Would love to get feedback from this community!

---

### r/kpop / r/kdrama Post
**Title:** Learn Korean vocabulary from your favorite K-Dramas and K-Pop songs

**Body:**
If you've ever wanted to understand what your favorite idols/actors are really saying, I made an app that teaches Korean vocabulary through K-Culture themes.

Twenty Korean has dedicated vocabulary sets for:
- K-Drama expressions (the ones you hear all the time!)
- K-Pop song lyrics vocabulary
- Korean food and restaurant phrases
- Internet slang that Koreans actually use

Plus an AI tutor named Dalli that you can practice Korean conversations with.

It teaches 20 words a day following the TOPIK curriculum, so you're actually building real language skills, not just memorizing random phrases.

Free to try!

---

## TikTok/Reels Script Ideas

### Script 1: "20 Words Challenge"
**Hook (0-3s):** "I'm learning Korean with just 20 words a day. Day 1."
**Body (3-12s):** Show app screen, flip through flashcards, show Korean words with pronunciation
**CTA (12-15s):** "Follow for daily Korean progress. App link in bio."

### Script 2: "K-Drama Words You Need"
**Hook (0-3s):** "5 Korean words every K-Drama fan needs to know"
**Body (3-25s):** Show each word with clip references:
1. 대박 (daebak) - "Amazing!"
2. 화이팅 (hwaiting) - "Fighting/You can do it!"
3. 아이고 (aigo) - "Oh my!"
4. 진짜 (jinjja) - "Really?!"
5. 오빠 (oppa) - "Older brother/term of endearment"
**CTA (25-30s):** "Learn 20 Korean words daily with Twenty Korean"

### Script 3: "AI Teaches Me Korean"
**Hook (0-3s):** "This AI speaks Korean better than my Korean teacher"
**Body (3-20s):** Screen record chatting with Dalli, show natural Korean voice response
**CTA (20-25s):** "Twenty Korean - free AI Korean tutor. Link in bio."

### Script 4: "Before/After"
**Hook (0-3s):** "Day 1 vs Day 30 of learning Korean"
**Body (3-15s):** 
- Day 1: Struggling with 안녕하세요
- Day 30: Having a conversation with AI in Korean
**CTA (15-20s):** "20 words a day adds up. Try Twenty Korean."

---

## Instagram Captions

### Post 1 (App Launch)
🇰🇷 Introducing Twenty Korean — Daily Korean in 20 Words.

Not hundreds of random words. Just 20 curated, high-frequency words daily — the ones native speakers actually use.

✅ 7,200+ TOPIK vocabulary
✅ AI conversation practice
✅ K-Drama & K-Pop themes
✅ Natural Korean voice
✅ Free to start

Your Korean journey starts with one word. Then 20. Then 7,200.

Link in bio 👆

#LearnKorean #KoreanLanguage #TOPIK #TwentyKorean #한국어 #한국어공부 #KDrama #KPop #LanguageLearning #StudyKorean

### Post 2 (K-Culture)
Ever watched a K-Drama and wished you understood what they were saying?

Twenty Korean teaches you the actual expressions used in Korean dramas, K-Pop songs, and everyday conversation.

20 words a day. That's it. That's the whole plan.

🎬 K-Drama vocabulary
🎵 K-Pop lyrics words
🍜 Korean food phrases
✈️ Travel essentials
💬 Internet slang

#KDrama #KPop #LearnKorean #TwentyKorean #한국어

### Post 3 (Progress Share)
POV: You just completed your 30-day streak on Twenty Korean 🔥

600 Korean words learned.
30 days of consistency.
1 step closer to understanding Korean without subtitles.

Share your streak! Tag us @TwentyKorean

#KoreanStudy #LanguageLearning #StudyMotivation #TwentyKorean

---

## Discord / Community Post

**Introducing Twenty Korean — A free Korean learning app**

Hey everyone! 👋

I wanted to share a Korean learning app I've been working on. It's called **Twenty Korean** and the concept is straightforward:

**Learn 20 Korean words per day, organized by TOPIK levels.**

Here's what's inside:
🧠 AI Korean Tutor "Dalli" — practice conversations anytime
📚 7,200 words across 6 TOPIK levels
🎬 K-Culture themes (K-Drama, K-Pop, K-Food, Travel, Slang)
🔤 Complete Hangeul alphabet learning
📊 Quizzes, sentence practice, and word networks
🏆 XP, achievements, and streaks

Free tier includes Level 1 (200 words), full Hangeul, and 5 AI chat messages.

Would love your feedback!

---

## ASO Keywords (Additional)

**Primary:** learn korean, korean language, topik, korean vocabulary, hangeul
**Secondary:** korean words, korean alphabet, korean flashcards, korean quiz, korean practice
**Long-tail:** learn korean for beginners, topik vocabulary app, korean word of the day, kdrama korean, kpop korean words
**Trending:** ai language learning, korean ai tutor, learn korean with ai, korean conversation practice
